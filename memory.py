...
class GiveHint:
    ...
    def update_name_counts(self, first_name_correct, last_name_correct, is_correct): # Aktualisiert die Zählung der richtig und falsch getippten Vor- und Nachnamen
        if is_correct:
            if first_name_correct:
                self.first_name_correct += 1
            else:
                self.first_name_incorrect += 1
            if last_name_correct:
                self.last_name_correct += 1
            else:
                self.last_name_incorrect += 1
        else:
            if not first_name_correct:
                self.first_name_incorrect += 1
            if not last_name_correct:
                self.last_name_incorrect += 1
...
class Memory:
    ...
    def handle_correct_guess(self):
        ...
        self.error_count = 0
        self.statistics.total_rounds += 1  # Runde zählt, wenn korrekt geraten
        self.root.after(Memory.WAIT_TIME_BEFORE_NEXT, self.load_next_image) # Warte WAIT_TIME_BEFORE_NEXT und lade dann das nächste Bild
    ...
    def hint_first_name(self):
        ...
        self.update_round_label() # stimmt dies hier?
    ...
    def solve_guess(self):
       # Zeigt den richtigen Namen an und bereitet das Spiel für die nächste Runde vor.
        last_name, first_name = self.current_folder.split('-')
        self.output_label.config(text=f"Name: {first_name} {last_name}")
        
        # Update statistics
        self.statistics.total_rounds += 1  # Eine neue Runde
        self.statistics.first_name_incorrect += 1  # Nicht gelöster Vorname
        self.statistics.last_name_incorrect += 1  # Nicht gelöster Nachname

        # Lädt das nächste Bild nach einer kurzen Verzögerung
        self.root.after(Memory.WAIT_TIME_BEFORE_NEXT, self.load_next_image)
    ...
    def load_next_image(self): 
        folders = os.listdir(Memory.IMAGES_FOLDER) # Wenn es bereits gespielte Runden gibt, filtere die Ordner, um nur diejenigen zu behalten, die noch nicht erraten wurden.
        if self.statistics.total_rounds > 0:
            folders = [folder for folder in folders if folder not in self.guessed_folders]
        if self.statistics.total_rounds > 0:
    ...
    def update_statistics_on_success(self):
        ...
        self.update_name_statistics(True)  # Neue Methode für die Aktualisierung der Namensstatistik

    def update_statistics_on_failure(self):
        self.statistics.total_incorrect += 1
        self.update_name_statistics(False)  # Neue Methode für die Aktualisierung der Namensstatistik

    def update_name_statistics(self, is_correct): # Aktualisiert die Statistiken für Vor- und Nachnamen basierend auf der Korrektheit des Tipps
        guess_parts = self.entry.get().split()    
        if len(guess_parts) >= 1:  # Überprüfen Sie, ob mindestens ein Name eingegeben wurde
            # Bestimmen Sie die korrekten Teile des Namens aus dem aktuellen Ordner
            first_name_correct = self.current_folder.split('-')[1]
            last_name_correct = self.current_folder.split('-')[0]

            # Prüfen Sie die Übereinstimmung mit den eingegebenen Teilen
            if len(guess_parts) >= 2:
                first_name_match = SimilarityCheck.check_similarity(guess_parts[0], first_name_correct)
                last_name_match = SimilarityCheck.check_similarity(guess_parts[-1], last_name_correct)
            else:
                # Wenn nur ein Teil eingegeben wurde, versuchen Sie, es sowohl mit Vor- als auch mit Nachname zu vergleichen
                first_name_match = SimilarityCheck.check_similarity(guess_parts[0], first_name_correct)
                last_name_match = SimilarityCheck.check_similarity(guess_parts[0], last_name_correct)

            # Aktualisieren Sie die Zählungen basierend auf Übereinstimmungen
            if first_name_match:
                if is_correct:
